...
fn usage_output() -> CliOutput {
    CliOutput {
        exit_code: 64,
        stdout: String::new(),
        stderr: "usage: foxprox validate-config <path> | default-config <sandbox-id> | plan-bwrap <config> -- <target...> | run-bwrap-tcp-egress <config> <stack-ip:port> <host-ip:port> -- <target...>\n".to_string(),
    }
}
