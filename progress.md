...
## 2026-06-22T14:40:00Z — Final alpha-scope verification sweep

- Command executed: `cargo fmt --all && cargo test --all && cargo run -p foxprox-cli --bin foxprox-lab -- run all >/tmp/foxprox-alpha-all-final.jsonl && cargo run -p foxprox-cli --bin foxprox-lab -- run robustness >/tmp/foxprox-alpha-robust-final.jsonl && cargo build -p foxprox-setup --bin foxproxsetup && cargo build -p foxprox-cli --bin foxprox-lab && target/debug/foxprox-lab run handoff-smoke >/tmp/foxprox-alpha-handoff.jsonl && target/debug/foxprox-lab run writeback-smoke >/tmp/foxprox-alpha-writeback.jsonl && target/debug/foxprox-lab run ping-smoke >/tmp/foxprox-alpha-ping.jsonl && target/debug/foxprox-lab run udp-forward-smoke >/tmp/foxprox-alpha-udp-forward.jsonl && target/debug/foxprox-lab run udp-deny-smoke >/tmp/foxprox-alpha-udp-deny.jsonl && target/debug/foxprox-lab run quic-smoke >/tmp/foxprox-alpha-quic.jsonl && target/debug/foxprox-lab run dns-smoke >/tmp/foxprox-alpha-dns.jsonl && target/debug/foxprox-lab run direct-dns-deny-smoke >/tmp/foxprox-alpha-direct-dns-deny.jsonl && target/debug/foxprox-lab run dns-attribution-smoke >/tmp/foxprox-alpha-dns-attr.jsonl && target/debug/foxprox-lab run tcp-syn-smoke >/tmp/foxprox-alpha-tcp-syn.jsonl && target/debug/foxprox-lab run tcp-synack-smoke >/tmp/foxprox-alpha-tcp-synack.jsonl && target/debug/foxprox-lab run tcp-bridge-smoke >/tmp/foxprox-alpha-tcp-bridge.jsonl && target/debug/foxprox-lab run tcp-bridge-http-deny-smoke >/tmp/foxprox-alpha-http-deny.jsonl && target/debug/foxprox-lab run tls-sni-deny-smoke >/tmp/foxprox-alpha-tls-deny.jsonl && target/debug/foxprox-lab run tcp-bridge-deny-smoke >/tmp/foxprox-alpha-tcp-bridge-deny.jsonl && target/debug/foxprox-lab run http-proxy-smoke >/tmp/foxprox-alpha-http-proxy.jsonl && target/debug/foxprox-lab run https-connect-smoke >/tmp/foxprox-alpha-connect.jsonl && target/debug/foxprox-lab run socks5-smoke >/tmp/foxprox-alpha-socks.jsonl && target/debug/foxprox-lab run proxy-deny-smoke >/tmp/foxprox-alpha-proxy-deny.jsonl && echo OK`
- Environment assumptions: Linux bwrap/TUN fd handoff works locally; real sandbox processes are permitted; `ping-smoke` grants `CAP_NET_RAW` specifically for real ping; all egress/proxy/DNS/TCP fixtures are loopback/local and no external network services are required.
- Expected result: all documented harness-lab alpha capabilities pass together: setup/handoff, packet write-back and real ping, UDP allow/deny, broker DNS and direct DNS deny, DNS attribution, transparent TCP SYN/SYN-ACK/bridge allow, transparent HTTP Host/path allow and deny, transparent TLS SNI deny/logging, QUIC candidate forwarding/logging, explicit HTTP/CONNECT/SOCKS allow and proxy deny/fail-closed paths, robustness/fail-closed scenarios, and shared broker/runtime/device/egress factoring.
- Observed result: pass. Workspace tests passed (`foxprox-broker` 5, `foxprox-core` 64, `foxprox-device` 6, `foxprox-egress` 1, `foxprox-cli` 2, `foxproxsetup` 4). Deterministic `all` and `robustness` groups passed, every listed real sandbox smoke completed, and the command printed final `OK`.
- Relevant output excerpt: `broker_denies_direct_external_dns_before_udp_egress ... ok`; `runtime::tests::tls_sni_dns_mismatch_fails_closed ... ok`; final `OK`.
- Changed files: `progress.md`.
- Interpretation: The documented harness-lab alpha scope is complete under local verification. Remaining work is post-alpha production hardening: async long-lived broker lifecycle integration, broader TCP bridge orchestration APIs, upstream DNS forwarding, configuration UX, and broader fuzz/property testing.
...
## 2026-06-22T15:35:00Z — Final alpha verification including long-lived broker session

- Command executed: `cargo fmt --all && cargo test --all && cargo run -p foxprox-cli --bin foxprox-lab -- run all >/tmp/foxprox-alpha2-all.jsonl && cargo run -p foxprox-cli --bin foxprox-lab -- run robustness >/tmp/foxprox-alpha2-robust.jsonl && cargo build -p foxprox-setup --bin foxproxsetup && cargo build -p foxprox-cli --bin foxprox-lab && target/debug/foxprox-lab run broker-session-smoke >/tmp/foxprox-alpha2-broker-session.jsonl && target/debug/foxprox-lab run ping-smoke >/tmp/foxprox-alpha2-ping.jsonl && target/debug/foxprox-lab run quic-smoke >/tmp/foxprox-alpha2-quic.jsonl && target/debug/foxprox-lab run direct-dns-deny-smoke >/tmp/foxprox-alpha2-direct-dns.jsonl && target/debug/foxprox-lab run tcp-bridge-smoke >/tmp/foxprox-alpha2-tcp-bridge.jsonl && target/debug/foxprox-lab run tcp-bridge-http-deny-smoke >/tmp/foxprox-alpha2-http-deny.jsonl && target/debug/foxprox-lab run tls-sni-deny-smoke >/tmp/foxprox-alpha2-tls-deny.jsonl && target/debug/foxprox-lab run http-proxy-smoke >/tmp/foxprox-alpha2-http-proxy.jsonl && target/debug/foxprox-lab run https-connect-smoke >/tmp/foxprox-alpha2-connect.jsonl && target/debug/foxprox-lab run socks5-smoke >/tmp/foxprox-alpha2-socks.jsonl && target/debug/foxprox-lab run proxy-deny-smoke >/tmp/foxprox-alpha2-proxy-deny.jsonl && echo OK`
- Environment assumptions: Linux bwrap/TUN fd handoff works locally; real sandbox processes are permitted; `broker-session-smoke` and `ping-smoke` grant `CAP_NET_RAW` for real ping; all egress/proxy/DNS/TCP fixtures are local and no external network services are contacted.
- Expected result: the clarified docs/initial-impl.md alpha scope passes including one long-lived mixed broker session plus focused deterministic and real sandbox checks for setup, TUN packet read/write, TCP/UDP/DNS/ICMP/QUIC, transparent HTTP/TLS inspection, explicit proxy frontends, denial/fail-closed behavior, and shared broker/runtime/device/egress boundaries.
- Observed result: pass. Workspace tests passed (`foxprox-broker` 5, `foxprox-core` 64, `foxprox-device` 6, `foxprox-egress` 1, `foxprox-cli` 2, `foxproxsetup` 4). Deterministic `all` and `robustness` groups passed. `broker-session-smoke` plus focused ping/QUIC/direct-DNS/TCP/proxy smokes all completed; final command printed `OK`.
- Relevant output excerpt: `broker_denies_direct_external_dns_before_udp_egress ... ok`; final `OK`. Stored smoke outputs include `/tmp/foxprox-alpha2-broker-session.jsonl` with all mixed-session checks true.
- Changed files: `progress.md`.
- Interpretation: Alpha scope in `docs/initial-impl.md` is now implemented and verified, including long-lived broker lifecycle behavior for mixed transparent traffic. Remaining items are beyond alpha: ergonomic product CLI/config, upstream DNS forwarding to real resolvers, hardening/fuzz scale-out, and production async/runtime performance work.
- Recent broker-session commit hash: `499027e`.
- Next verification gap: none for alpha. Future work should be tracked as post-alpha/productization.
- Commit hash after commit: pending.

## 2026-06-22T16:30:00Z — Usable bwrap sandbox alpha prototype

- Gap selected: the prior work proved alpha behavior with smokes, but did not provide a user-facing command that runs an arbitrary target in a real bwrap network namespace with a long-lived host broker.
- Implementation:
  - Added `foxprox-lab sandbox ... -- target args...` as the alpha host launcher.
  - The launcher starts bwrap with `--unshare-net`, temporary setup capabilities, `/dev/net/tun`, a per-session handoff socket, sandbox `/etc/resolv.conf` pointed at `10.0.2.1`, and `foxproxsetup -- target` as the namespace entrypoint.
  - The host launcher receives the TUN fd, runs one long-lived broker loop until the target exits, emits JSON-lines audit, and cleans up the session directory.
  - Wired transparent ICMP/DNS/UDP/QUIC through `TransparentBroker`, transparent TCP byte bridging through `TransparentTcpBridgeRuntime`, and sandbox-reachable HTTP proxy (`10.0.2.1:8080`) plus SOCKS5 (`10.0.2.1:1080`) over the same TUN/smoltcp bridge.
  - Added real host UDP egress adapter `HostUdpEgress` with optional sandbox-IP to host-IP mapping for local fixture tests.
  - Added upstream DNS forwarding support for broker DNS misses, plus DNS A-answer extraction for cache attribution; local `--dns-answer host=ip` remains available for deterministic fixtures.
  - Documented alpha sandbox usage and limitations in `README.md`.
- Command executed: `cargo fmt --all && cargo test --all && cargo run -p foxprox-cli --bin foxprox-lab -- run all >/tmp/foxprox-alpha3-all.jsonl && cargo run -p foxprox-cli --bin foxprox-lab -- run robustness >/tmp/foxprox-alpha3-robust.jsonl && cargo build -p foxprox-setup --bin foxproxsetup -p foxprox-cli --bin foxprox-lab && python3 /tmp/foxprox-alpha-prototype-check.py >/tmp/foxprox-alpha3-prototype.out 2>/tmp/foxprox-alpha3-prototype.err && target/debug/foxprox-lab run broker-session-smoke >/tmp/foxprox-alpha3-broker-session.jsonl && target/debug/foxprox-lab run ping-smoke >/tmp/foxprox-alpha3-ping.jsonl && target/debug/foxprox-lab run quic-smoke >/tmp/foxprox-alpha3-quic.jsonl && target/debug/foxprox-lab run direct-dns-deny-smoke >/tmp/foxprox-alpha3-direct-dns.jsonl && target/debug/foxprox-lab run tcp-bridge-http-deny-smoke >/tmp/foxprox-alpha3-http-deny.jsonl && target/debug/foxprox-lab run tls-sni-deny-smoke >/tmp/foxprox-alpha3-tls-deny.jsonl && target/debug/foxprox-lab run http-proxy-smoke >/tmp/foxprox-alpha3-http-proxy.jsonl && target/debug/foxprox-lab run https-connect-smoke >/tmp/foxprox-alpha3-connect.jsonl && target/debug/foxprox-lab run socks5-smoke >/tmp/foxprox-alpha3-socks.jsonl && echo OK`
- Prototype check details: `/tmp/foxprox-alpha-prototype-check.py` runs `target/debug/foxprox-lab sandbox --allow-all --proxy-env --no-upstream-dns --egress-map 203.0.113.22=127.0.0.1 --dns-answer lab.example=203.0.113.22 --timeout-secs 40 -- /usr/bin/python3 -c <script>` against a local TCP fixture. The sandbox script verifies real ping to `10.0.2.1`, broker DNS resolution for `lab.example`, transparent TCP to `203.0.113.22`, explicit HTTP proxy via `10.0.2.1:8080`, and SOCKS5 TCP CONNECT via `10.0.2.1:1080`.
- Observed result: pass. Workspace tests passed (`foxprox-broker` 5, `foxprox-core` 64, `foxprox-device` 6, `foxprox-egress` 1, `foxprox-cli` 2, `foxproxsetup` 4). Deterministic `all` and `robustness` passed. The usable prototype check printed `usable prototype ok`, saw all 3 host fixture requests, and audit showed `icmp_message`, broker `dns_query`, transparent `tcp_connect_attempt`/`tcp_flow_closed`, `http_request` from `http_proxy`, `socks_connect` from `socks5`, and `sandbox_exited` allow. Focused broker/session/ping/QUIC/direct-DNS/HTTP/TLS/proxy smokes passed and the final command printed `OK`.
- Relevant output excerpt from `/tmp/foxprox-alpha3-prototype.out`: `64 bytes from 10.0.2.1 ...`; `usable prototype ok`.
- Relevant audit excerpt from `/tmp/foxprox-alpha3-prototype.err`: `"event":"http_request","frontend":"http_proxy","protocol":"http","destination":"127.0.0.1` and `"event":"socks_connect","frontend":"socks5","protocol":"socks","destination":"127.0.0.1`.
- Changed files: `README.md`, `crates/foxprox-cli/src/main.rs`, `crates/foxprox-core/src/dns.rs`, `crates/foxprox-core/src/runtime.rs`, `crates/foxprox-egress/src/lib.rs`, `progress.md`.
